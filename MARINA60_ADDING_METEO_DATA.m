...
if iec_format % Create a functional date vector in ISO 8601 format if IEC 62862-1-3 format is required
    time_func = (datetime([2015 1 1 0 0 0]):minutes(60/num_obs_report):...
        datetime([2015 12 31 23 60-60/num_obs_report 0]))'; % Functional date (IEC 62862-1-3)
...
elseif num_obs>num_obs_report % Higher sampling frequency that the wanted for printing
    dim = size(SERIES_out_int);
    SERIES_out_sim = NaN(8760*num_obs_report,dim(2),dim(3));
    for i = 1:num_series
        SERIES_out_sim(:,:,i) = reduce_intrahour_freq(SERIES_out_int(:,:,i),num_obs,num_obs_report,6); % Only the additional meteorological variables are considered
    end
    
    if num_obs_report==num_obs_meteo % Same sampling frequency
        METEO_out = meteonormData(:,8:end); % Only the additional meteorological variables are considered
    elseif num_obs_report>num_obs_meteo % Interpolate to keep the original higher sampling
        xq = transpose(1:8760*num_obs_report);
        meteoFreq = num_obs_report/num_obs_meteo;
        x = transpose(1:meteoFreq:8760*num_obs_report); x = [x; xq(end)]; % Add a row to interpolate the last hour
        METEO_out = interp1(x,[meteonormData(:,8:end); meteonormData(end,8:end)],xq,'linear'); % Only the additional meteorological variables are interpolated
    else % Reduce the intrahourly frequency to match the radiation sampling frequency
        METEO_out = reduce_intrahour_freq(meteonormData(8:end),num_obs_meteo,num_obs_report,0); % Only the additional meteorological variables are considered
    end
else
    fprintf('Sampling frequncy for printing (%d) must be equal or lesser than the data sampling frequency (%d)\n',...
        num_obs_report,num_obs)
    return
end
...
%     end
    ...
    %% Add the meteo data
    SERIES_out_sim(:,13:end,i) = METEO_out(:,cols); % Columns of the wanted variables

    %% Write down SAM CSV format ------------------------------------------
    if sam_format
        filename_out = strcat(path_series,'\','SAM_',namef,'_',name_series{i},'.csv');
        sam_out = SERIES_out_sim(:,[1:5,7,9,11,13:end],i); % [Year Month Day Hour Minute GHI DNI DHI t_air rh bp ws]
        % Continuous day in the month along the year (override day substitutions for final csv file)
        m31 = zeros(1,31*24*num_obs_report); l = 1;
        for d = 1:31
            for o = 1:24*num_obs_report
                m31(1,l) = d;
                l = l+1;
            end
        end
        m30 = m31(1,1:30*24*num_obs_report); m28 = m31(1,1:28*24*num_obs_report);
    ...
    if iec_format
        ...
        iec_out = SERIES_out_sim(:,[1:6,7,9,11,13:end],i); % % [Year Month Day Hour Minute Second GHI DNI DHI t_air rh bp ws]
        ...
        iec_write(filename_out,iec_out,time_func_str,num_obs_report,options_iec); % Function
    end
