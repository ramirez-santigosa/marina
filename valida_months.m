function [daily,monthly,subst,nonvalid_m] = valida_months(input_daily,max_nonvalid,max_dist)
...
%
%   OUTPUT:
%   daily: Updated daily radiation values, if substitutions were made.
%   Columns:
...
%   subst: Substitutions in a year. Columns:
%       1 - Month
%       2 - Origin day
%       3 - Substituted day
...
num_s = 0; % Number of substitutions
subst = zeros(12*max_nonvalid,3); % Pre-allocation (max number of substitutions in a year)
...
for m = 1:12
    ...
    if num_nvDNI==0 % If all days in DNI series are valid -----------------
        ...
        if num_nvGHI==0
            ...
            % values aren't. GHI values cannot be substituted.
            rad_monthGHI = NaN;
    ...
    elseif num_nvDNI<=max_nonvalid % If DNI non-valid days are less than the allowed ---
        ...
        subst_m = zeros(num_nvDNI,3); % To save substitutions in the month
        ...
        for i = 1:num_nvDNI
            ...
            days_m(nvDNI_days(i)) = days_range(i_min_dist); % Substitute non-valid day with the closest to the mean
            DNI(nvDNI_days(i)) = DNI(days_range(i_min_dist)); % Update DNI value
            fDNI(nvDNI_days(i)) = 2; % Update the daily flag
            subst_m(i,:) = [m days_range(i_min_dist) nvDNI_days(i)]; % Save substitutions in the month
        end
        rad_monthDNI = sum(DNI); % Monthly DNI is sum up of the daily values
        f_mvalDNI = 1; % Monthly DNI flag !!!
        ...
        if sum(validGHI)==sum(validDNI) % Equal number of valid days !!!Pero no implica que sean los mismos?
            % The substitutions did in DNI treatment are repeated in GHI
            % treatment, it is, GHI values in the positions of non-valid
            % days in DNI series are substituted with the GHI value in the
            % position of the closest to the mean DNI daily value.
            GHI(pos_nvDNI) = GHI(subst_m(:,2));
            fGHI(pos_nvDNI) = 2; % Update the daily flag
            rad_monthGHI = sum(GHI); % Monthly GHI is sum up of the daily values
            f_mvalGHI = 1; % Monthly GHI flag !!!
        else
            % In this case, some GHI values aren't acceptable. GHI values cannot be substituted.
            rad_monthGHI = NaN;
            f_mvalGHI = 5; % Monthly GHI flag !!!
        end
        
        % Save substitutions along the year
        subst(num_s+1:num_s+num_nvDNI,:) = subst_m;
        num_s = num_s+num_nvDNI; % Update global number of substitutions in the year
    ...
    % Update daily values after possible substitutions
...
subst(num_s+1:end,:) = []; % Shrink substitutions matrix
