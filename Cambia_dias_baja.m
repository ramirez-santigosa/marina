function [resultado,usados,cambiados,contador,control]...
    =Cambia_dias_baja(mes,A,VMR,max_cambios,dist_dias,max_uso)
...
if control > lim  % Condicion de estar por fuera del limte establecido
    while (control > lim && contador<=max_cambios)
        ...
        [maximo,posicionmax]=max((resultado(:,col_val_ini).*~cambiados));  % Valor maximo de radiación y su posicion 
        pos_prim=posicionmax(1)-dist_dias;
        pos_ultm=posicionmax(1)+dist_dias;
        ...
        incremento=(valores_posibles-resultado(posicionmax,col_val_ini));
